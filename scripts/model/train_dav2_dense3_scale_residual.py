#!/usr/bin/env python3
"""Six-epoch dense3 global-scale plus dense-residual log-depth training."""
...
from bim_priorda3.models.dav2_dense3_scale_residual import (
...
def evaluate(model, loader, device, amp):
    model.eval()
    metrics = {name: BothMetrics() for name in ("raw_da3", "scaled_depth", "final")}
...
def main():
    ...
    runtime={"epochs":epochs,"micro_batch":int(cfg.train.batch_size),"accumulation":accumulation,
             ...
             "loss":"0.5 pixel-micro + 0.5 frame-macro absolute log-depth; final depth only",
    ...
    for epoch in range(1,epochs+1):
        ...
        samples=0
        sums={"log_depth":0.,"pixel_micro_log_depth":0.,"frame_macro_log_depth":0.}
        ...
        for index,raw_batch in enumerate(train_loader,1):
            ...
            with torch.autocast(device_type=device.type,dtype=torch.float16,enabled=amp):
                output=model(batch)
                losses=dense_log_depth_loss(output["depth"],batch["gt_depth"],batch["gt_valid"])
            ...
            n=batch["rgb"].shape[0]
            samples+=n
            sums["log_depth"]+=float(losses["total"].detach())*n
            for k in ("pixel_micro_log_depth","frame_macro_log_depth"):
            ...
            if index==1 or index%int(cfg.train.log_every)==0 or index==len(train_loader):
                print(f"epoch={epoch}/{epochs} batch={index}/{len(train_loader)} log_depth={float(losses['total'].detach()):.6f} samples_per_s={samples/(time.perf_counter()-started):.2f} optimizer_steps={steps} skipped={skipped}",flush=True)
