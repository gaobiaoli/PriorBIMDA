...
run_name=stanford_area1_f36_adapter_3res32_3res64_effective_batch16_6epoch_20260906
...
for scene in hxp 759 1px; do
  result_dir="$project_root/results/matterport3d/${scene}_f36_adapter_3res32_3res64_20260906_zero_shot"
...

{
  ...
  echo "config=$config"
  echo "architecture=adapter_3_to_32_3res_to_64_3res_to_128_r36_head_128_to_64_to_1"
  echo "micro_batch=2"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_adapter_3res32_3res64_pipeline_20260906.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
