...
run_name=stanford_area1_f36_anchor_tf32_fast_sdp_batch32_6epoch_20260905
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_anchor_tf32_fast_sdp_batch32_full_pipeline_20260905.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
...
echo "micro_batch=32 gradient_accumulation=1 effective_batch=32 log_every=20 source_snapshot=false"
...
for scene in hxp 759 1px; do
  zero_shot_dir="$project_root/results/matterport3d/${scene}_f36_anchor_tf32_fast_sdp_batch32_6epoch_20260905_zero_shot"
