...
run_name=stanford_area1_dense3_residual_logdepth_effective_batch16_6epoch_20260907
...
echo "architecture=dense3_single_uncentered_dense_r504_no_scale_head"
echo "condition=masked_log_bim,log_metric_da3,bim_mask disagreement=false"
echo "formula=D_final=D_DA3*exp(r504)"
...
run_command "$python_bin" scripts/model/train_dav2_dense3_residual.py --config "$config" --device cuda
for checkpoint_name in best latest; do
  for scene in hxp 759 1px; do
    ...
    run_command "$python_bin" scripts/model/evaluate_matterport_bimnet_scale_refiner.py \
      ...
      --output-dir "$project_root/results/matterport3d/${scene}_dense3_residual_logdepth_${checkpoint_name}_20260907_zero_shot" \
