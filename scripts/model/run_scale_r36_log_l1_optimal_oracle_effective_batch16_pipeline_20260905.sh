...
run_name=stanford_area1_scale_r36_log_l1_optimal_oracle_effective_batch16_6epoch_20260905
...
for scene in hxp 759 1px; do
  zero_shot_dir="$project_root/results/matterport3d/${scene}_scale_r36_log_l1_optimal_oracle_effective_batch16_6epoch_20260905_zero_shot"
...

{
  ...
  echo "oracle_scale_estimator=weighted_log_l1_optimal"
  echo "scale_teacher_mode=scalar_smooth_l1"
  echo "scale_teacher_weight=0.5"
  echo "scale_equivariance_weight=0.1"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_scale_r36_log_l1_optimal_oracle_effective_batch16_pipeline_20260905.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
...
echo "architecture=original_scale_r36_no_post_scale_adapter"
echo "oracle_scale_estimator=weighted_log_l1_optimal scale_teacher=scalar_smooth_l1"
echo "scale_teacher_weight=0.5 effective_batch=16"
