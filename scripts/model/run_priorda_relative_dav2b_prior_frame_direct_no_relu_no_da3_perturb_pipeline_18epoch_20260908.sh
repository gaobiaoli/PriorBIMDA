...
run_name=stanford_area1_priorda_relative_dav2b_prior_frame_direct_no_relu_no_da3_perturb_effective_batch16_18epoch_20260908
...
if [[ -e "$log_path" || -e "$output_dir/receipt.json" ]]; then
  echo "Refusing to overwrite direct PriorDA prior-frame no-ReLU 18-epoch controlled experiment" >&2
  exit 2
fi
...
echo "architecture=DAv2-relative-ViT-B14+DPT+Identity-no-final-ReLU+zero-init-condition-only"
echo "condition=q_prior_normalized,q_BIM_in_prior_frame,BIM_mask"
echo "normalization=D_prior_min_range_shared_by_conditions_and_direct_output_denormalization"
echo "output=prior_minimum+prior_range/signed_DAv2_relative_disparity no_output_adapter no_scale_shift"
echo "micro_batch=2 accumulation=8 effective_batch=16 epochs=18"
echo "optimizer=AdamW encoder_lr=5e-6 decoder_and_condition_lr=5e-5 scheduler=cosine"
echo "mixed_precision=bfloat16 gradient_scaler=false signed_output_with_priorDA_positive_reciprocal_rule"
...
for checkpoint_name in best latest; do
  for scene in hxp 759 1px; do
    echo "PRIOR_FRAME_DIRECT_NO_RELU_ZERO_SHOT checkpoint=$checkpoint_name scene=$scene started=$(date -Ins)"
    run_command "$python_bin" scripts/model/evaluate_matterport_bimnet_scale_refiner.py \
      ...
      --output-dir "$project_root/results/matterport3d/${scene}_priorda_relative_dav2b_prior_frame_direct_no_relu_no_da3_perturb_${checkpoint_name}_20260908_zero_shot" \
