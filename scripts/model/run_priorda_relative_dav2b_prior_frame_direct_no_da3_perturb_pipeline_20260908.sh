...
run_name=stanford_area1_priorda_relative_dav2b_prior_frame_direct_no_da3_perturb_effective_batch16_6epoch_20260908
...
if [[ -e "$log_path" || -e "$output_dir/receipt.json" ]]; then
  echo "Refusing to overwrite direct PriorDA prior-frame controlled experiment" >&2
  exit 2
fi
...
echo "architecture=unaltered-DAv2-relative-ViT-B14+DPT+ReLU+zero-init-condition-only"
...
echo "output=prior_minimum+prior_range/official_DAv2_relative_disparity no_output_adapter no_scale_shift"
...
for checkpoint_name in best latest; do
  for scene in hxp 759 1px; do
    echo "PRIOR_FRAME_DIRECT_ZERO_SHOT checkpoint=$checkpoint_name scene=$scene started=$(date -Ins)"
    run_command "$python_bin" scripts/model/evaluate_matterport_bimnet_scale_refiner.py \
      ...
      --output-dir "$project_root/results/matterport3d/${scene}_priorda_relative_dav2b_prior_frame_direct_no_da3_perturb_${checkpoint_name}_20260908_zero_shot" \
