...
run_name=stanford_area1_da3metric_large_dense3_residual_aux72_logdepth_effective_batch16_6epoch_20260907
...
if [[ -e "$log_path" || -e "$output_dir/receipt.json" ]]; then
  echo "Refusing to overwrite DA3Metric-Large dense3 experiment" >&2
  exit 2
fi
...
echo "foundation=DA3Metric-Large_DINOv2-L14_DPT256 official_metric_base_model=unavailable"
...
run_command "$python_bin" -m pytest -q tests/test_dav2_dense3_residual.py
...
for checkpoint_name in best latest; do
  for scene in hxp 759 1px; do
    ...
    run_command "$python_bin" scripts/model/evaluate_matterport_bimnet_scale_refiner.py \
      ...
      --output-dir "$project_root/results/matterport3d/${scene}_da3metric_large_dense3_residual_aux72_${checkpoint_name}_20260907_zero_shot" \
