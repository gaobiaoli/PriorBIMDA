...
run_name=stanford_area1_f36_adapter_resblocks3_hard_centered_r36_effective_batch16_6epoch_20260906
...
for scene in hxp 759 1px; do
  result_dir="$project_root/results/matterport3d/${scene}_f36_adapter_resblocks3_hard_centered_r36_20260906_zero_shot"
...

{
  ...
  echo "config=$config"
  echo "architecture=adapter_hidden32_resblocks3_r36_head_128_to_64_to_1_hard_centered_native36"
  echo "r36_projection=r36_minus_spatial_mean_native36"
  echo "residual_zero_mean_weight=0.1"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_adapter_resblocks3_hard_centered_r36_pipeline_20260906.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
