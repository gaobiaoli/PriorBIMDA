...
run_name=stanford_area1_f36_adapter_resblocks3_main_path_da3_scale_perturb_no_equivariance_effective_batch16_6epoch_20260906
...
for scene in hxp 759 1px; do
  result_dir="$project_root/results/matterport3d/${scene}_f36_adapter_resblocks3_main_path_da3_scale_perturb_20260906_zero_shot"
...

{
  ...
  echo "r36_hard_mean_center=false"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_adapter_resblocks3_main_path_da3_scale_perturb_pipeline_20260906.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
...
echo "architecture=adapter_hidden32_resblocks3_r36_head_128_to_64_to_1"
echo "main_path_da3_scale_perturbation=q_exp_uniform_minus0.2_plus0.2_probability1"
echo "auxiliary_scale_equivariance=false r36_hard_mean_center=false"
