...
def build_batch(
    *,
    ...
    if isinstance(model, (BIMEarlyFusionDAv2JointScaleLow, DAv2Dense4, PriorDARelativeMetricRefiner, PriorDARelativePriorFrameMetricRefiner, PriorDARelativePriorFrameNoReLUMetricRefiner, PriorDARelativePriorIdentityMetricRefiner, PriorDARelativeZeroAnchorMetricRefiner, DAv2Dense3Residual, DAv2Dense3ScaleResidual, DA3MetricDense3ResidualAux72)):
...
def evaluate_frame(
    *,
    ...
    rgb = cv2.resize(
        ...
        interpolation=cv2.INTER_CUBIC if isinstance(model, (PriorDARelativeMetricRefiner, PriorDARelativePriorFrameMetricRefiner, PriorDARelativePriorFrameNoReLUMetricRefiner, PriorDARelativePriorIdentityMetricRefiner, PriorDARelativeZeroAnchorMetricRefiner)) else cv2.INTER_AREA,
    )
    ...
    dense4 = isinstance(model, (DAv2Dense4, PriorDARelativeMetricRefiner, PriorDARelativePriorFrameMetricRefiner, PriorDARelativePriorFrameNoReLUMetricRefiner, PriorDARelativePriorIdentityMetricRefiner, PriorDARelativeZeroAnchorMetricRefiner, DAv2Dense3Residual, DA3MetricDense3ResidualAux72))
    ...
    elif dense3_scale_residual:
        scale_low_process = scale_process
    ...
    learned_scale = None if dense4 else (
        ...
        if isinstance(model, (BIMEarlyFusionDAv2JointScaleLow, PriorBIMDATwoStage, DAv2Dense3ScaleResidual))
    ...
    *,
    ...
    return {
        ...
        "model": {
            ...
            "architecture": ((model.ARCHITECTURE if isinstance(model, (PriorDARelativeMetricRefiner, PriorDARelativePriorFrameMetricRefiner, PriorDARelativePriorFrameNoReLUMetricRefiner, PriorDARelativePriorIdentityMetricRefiner, PriorDARelativeZeroAnchorMetricRefiner)) else None) or (DAv2Dense4.ARCHITECTURE if isinstance(model, DAv2Dense4) else None) or (DA3MetricDense3ResidualAux72.ARCHITECTURE if isinstance(model, DA3MetricDense3ResidualAux72) else None) or (DAv2Dense3ResidualAux72.ARCHITECTURE if isinstance(model, DAv2Dense3ResidualAux72) else None) or (DAv2Dense3Residual.ARCHITECTURE if isinstance(model, DAv2Dense3Residual) else None) or (DAv2Dense3ScaleResidual.ARCHITECTURE if isinstance(model, DAv2Dense3ScaleResidual) else None)) or iterative_geometry_architecture or (
                (
                    (
...
def main() -> None:
    ...
    if cfg.model.get("dense3_residual", {}).get("enabled", False):
        ...
        if checkpoint["config"]["model"] != dict(cfg.model):
            raise ValueError("Dense3 evaluation model config differs from the training checkpoint")
        dav2 = cfg.model.dav2
        dense = cfg.model.dense3_scale_residual
        model = DAv2Dense3ScaleResidual.from_pretrained(
            dav2.model_id,
            revision=dav2.revision,
            local_files_only=True,
            regression_hidden_size=int(dense.regression_hidden_size),
            head_dropout_probability=float(dense.head_dropout_probability),
            max_dense_log_residual=float(dense.max_dense_log_residual),
        )
        model.load_state_dict(checkpoint["model"], strict=True)
        PREDICTION_NAMES = ("raw", "scale", "final", "oracle_frame_scale")
    elif cfg.model.get("priorda_relative_prior_frame_no_relu_refiner", {}).get("enabled", False):
        if checkpoint.get("architecture") != PriorDARelativePriorFrameNoReLUMetricRefiner.ARCHITECTURE:
            raise ValueError("Expected the direct DAv2-relative no-ReLU PriorDA prior-frame checkpoint")
        if checkpoint["config"]["model"] != dict(cfg.model):
            raise ValueError("No-ReLU prior-frame evaluation model config differs from training")
        dav2 = cfg.model.dav2
        model = PriorDARelativePriorFrameNoReLUMetricRefiner.from_pretrained(
    ...
    if isinstance(model, PriorDARelativePriorFrameNoReLUMetricRefiner):
        summary["protocol"]["name"] = "frozen Area_1 direct DAv2-relative no-ReLU PriorDA prior-frame refinement zero-shot; three-rule valid frames"
        summary["protocol"]["aggregation"] = "frame-macro primary; pixel-micro secondary; no alignment"
        summary["model"]["output_head"] = "official pretrained DPT weights with final ReLU replaced by Identity; reciprocal then D_prior min-range de-normalization"
        summary["model"]["condition"] = dict(cfg.model.priorda_relative_prior_frame_no_relu_refiner)
    elif isinstance(model, PriorDARelativePriorFrameMetricRefiner):
        ...
        summary["protocol"]["aggregation"] = "frame-macro primary; pixel-micro secondary; no alignment"
        summary["model"]["output_head"] = "D_DA3*exp(uncentered r504); no scale head; all positive GT retained"
        summary["model"]["condition"] = dict(cfg.model.dense3_residual)
    elif isinstance(model, DAv2Dense3ScaleResidual):
        summary["protocol"]["name"] = "frozen Area_1 dense3 global-scale+dense-residual zero-shot; three-rule valid frames"
        summary["protocol"]["aggregation"] = "frame-macro primary; pixel-micro secondary; no alignment"
        summary["model"]["output_head"] = "D_DA3*exp(z_s)*exp(mean-centered r504); all positive GT retained"
        summary["model"]["condition"] = dict(cfg.model.dense3_scale_residual)
