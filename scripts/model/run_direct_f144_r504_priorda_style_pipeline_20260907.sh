...
run_name=stanford_area1_dav2_early_fusion_direct_f144_upsampled_r504_no_global_scale_6epoch
config="$project_root/configs/${run_name}_full_depth_metric_da3.yaml"
...
for checkpoint_label in best latest; do
  for scene in hxp 759 1px; do
    result_dir="$project_root/results/matterport3d/${scene}_direct_f144_r504_priorda_style_${checkpoint_label}_20260907_zero_shot"
...

{
  ...
  echo "config=$config"
  echo "training=fresh_6epoch_original_r18_only_schedule"
  echo "architecture=direct_metric_DA3_prior_plus_signed_r504_log_residual"
  echo "auxiliary_scale_equivariance=false"
  echo "evaluate_checkpoints=best,latest_epoch6"
  echo "micro_batch=2"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_direct_f144_r504_priorda_style_pipeline_20260907.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
