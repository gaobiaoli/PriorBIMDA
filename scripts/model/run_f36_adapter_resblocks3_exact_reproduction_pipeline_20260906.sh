...
run_name=stanford_area1_f36_adapter_resblocks3_exact_reproduction_20260906
...
for scene in hxp 759 1px; do
  result_dir="$project_root/results/matterport3d/${scene}_f36_adapter_resblocks3_exact_reproduction_20260906_zero_shot"
...

{
  ...
  echo "config=$config"
  echo "reference_core_config_sha256=34ef764e691dc7160534489910f4a2d7791beff87f524ccfdb7cc3f26a51c3d8"
  echo "architecture=f36_calibrated_disagreement_adapter_hidden32_resblocks3"
  echo "micro_batch=2"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_adapter_resblocks3_exact_reproduction_pipeline_20260906.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
