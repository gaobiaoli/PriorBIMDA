...
run_name=stanford_area1_priorda_relative_dav2b_bim_zero_anchor_no_da3_perturb_effective_batch16_6epoch_20260908
...
if [[ -e "$log_path" || -e "$output_dir/receipt.json" ]]; then
  echo "Refusing to overwrite zero-anchor controlled experiment" >&2
  exit 2
fi
...
echo "condition=Dmax_BIM/D_raw,Dmax_BIM/D_BIM,BIM_mask"
echo "normalization=physical_zero_origin valid_BIM_maximum_scale no_BIM_minimum_subtraction"
echo "output=Dmax_BIM/Softplus10_DPT_disparity no_BIM_minimum_floor no_zero_or_negative_disparity"
...
for checkpoint_name in best latest; do
  for scene in hxp 759 1px; do
    ...
    run_command "$python_bin" scripts/model/evaluate_matterport_bimnet_scale_refiner.py \
      ...
      --output-dir "$project_root/results/matterport3d/${scene}_priorda_relative_dav2b_bim_zero_anchor_no_da3_perturb_${checkpoint_name}_20260908_zero_shot" \
