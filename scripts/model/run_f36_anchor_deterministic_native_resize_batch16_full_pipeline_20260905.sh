...
run_name=stanford_area1_f36_anchor_deterministic_native_resize_batch16_6epoch_20260905
config="$project_root/configs/${run_name}.yaml"
output_dir="$project_root/outputs/$run_name"
results_dir="$project_root/results/stanford_area1/f36_anchor_deterministic_native_resize_batch16_6epoch_20260905"
...
source_snapshot="$tmp_dir/${run_name}_source_snapshot"
...
if [[ -e "$source_snapshot" ]]; then
  echo "Refusing to overwrite an existing source snapshot: $source_snapshot" >&2
  exit 2
fi

# Freeze the implementation for the delayed test and zero-shot subprocesses.
mkdir -p "$source_snapshot/scripts"
cp -a "$project_root/src" "$source_snapshot/src"
cp -a "$project_root/scripts/model" "$source_snapshot/scripts/model"
...
cd "$project_root"
export PYTHONPATH="$source_snapshot/src:$source_snapshot/scripts/model"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_anchor_deterministic_native_resize_batch16_full_pipeline_20260905.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
...
echo "micro_batch=16 gradient_accumulation=1 effective_batch=16"
run_command nvidia-smi --query-gpu=name,memory.total,memory.used,utilization.gpu --format=csv,noheader

run_command "$python_bin" "$source_snapshot/scripts/model/train_dav2_joint_scale_low.py" \
  --config "$config" \
  --device cuda \
  --deterministic
...
for scene in hxp 759 1px; do
  zero_shot_dir="$project_root/results/matterport3d/${scene}_f36_anchor_deterministic_native_resize_batch16_6epoch_20260905_zero_shot"
  echo "zero_shot_scene_started=$scene time=$(date -Ins)"
  run_command "$python_bin" "$source_snapshot/scripts/model/evaluate_matterport_bimnet_scale_refiner.py" \
