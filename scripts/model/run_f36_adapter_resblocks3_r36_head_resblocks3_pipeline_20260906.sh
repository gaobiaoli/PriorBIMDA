...
run_name=stanford_area1_f36_adapter_resblocks3_r36_head_resblocks3_effective_batch16_6epoch_20260906
...
for scene in hxp 759 1px; do
  result_dir="$project_root/results/matterport3d/${scene}_f36_adapter_resblocks3_r36_head_resblocks3_20260906_zero_shot"
...

{
  ...
  echo "config=$config"
  echo "architecture=adapter_hidden32_resblocks3_r36_head_hidden64_resblocks3"
  echo "micro_batch=2"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_f36_adapter_resblocks3_r36_head_resblocks3_pipeline_20260906.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
