#!/usr/bin/env python3
"""Six-epoch dense r504 training with native-F72 auxiliary supervision."""
...
from bim_priorda3.models.dav2_dense3_residual import dense_log_depth_loss
...
def evaluate(model, loader, device, amp):
    model.eval()
    metrics = {name: BothMetrics() for name in ("raw_da3", "auxiliary_r72", "final")}
    ...
    for raw_batch in loader:
        ...
        predictions = {
            ...
            "auxiliary_r72": output["auxiliary_depth72"],
            "final": output["depth"],
        }
...
def audit_depth_range(dataset):
    ...
    return {"split":"train", "frames":len(rows), "gt_valid_pixels":n,
            ...
            "policy":"DA3-anchored exponential scale+residual output; do not discard any GT"}
...
def main():
    ...
    if "dav3" in cfg.model:
        ...
        init = model.initialization_audit(device=device)
        ...
        foundation_checkpoint_sha256 = str(cfg.model.dav2.checkpoint_sha256)
    ...
    runtime={"epochs":epochs,"micro_batch":int(cfg.train.batch_size),"accumulation":accumulation,
             ...
             "loss":"L504 + auxiliary_weight*L72; each uses balanced micro/macro absolute log-depth",
    ...
    for epoch in range(1,epochs+1):
        ...
        samples=0
        sums={"total_log_depth":0.,"loss504":0.,"loss72":0.}
        ...
        for index,raw_batch in enumerate(train_loader,1):
            ...
            with torch.autocast(device_type=device.type,dtype=torch.float16,enabled=amp):
                output=model(batch)
                loss504=dense_log_depth_loss(output["depth"],batch["gt_depth"],batch["gt_valid"])
                loss72=dense_log_depth_loss(output["auxiliary_depth72"],batch["gt_depth"],batch["gt_valid"])
                total_loss=loss504["total"]+float(cfg.loss.auxiliary_r72_weight)*loss72["total"]
            if not bool(torch.isfinite(total_loss)):
                raise FloatingPointError(f"Nonfinite log-depth loss at epoch {epoch}, batch {index}")
            ...
            n=batch["rgb"].shape[0]
            samples+=n
            sums["total_log_depth"]+=float(total_loss.detach())*n
            sums["loss504"]+=float(loss504["total"].detach())*n
            sums["loss72"]+=float(loss72["total"].detach())*n
            for k in counts:
            ...
            if index==1 or index%int(cfg.train.log_every)==0 or index==len(train_loader):
                print(f"epoch={epoch}/{epochs} batch={index}/{len(train_loader)} total={float(total_loss.detach()):.6f} loss504={float(loss504['total'].detach()):.6f} loss72={float(loss72['total'].detach()):.6f} samples_per_s={samples/(time.perf_counter()-started):.2f} optimizer_steps={steps} skipped={skipped}",flush=True)
        ...
        if epoch in {int(value) for value in cfg.train.get("checkpoint_epochs", [])}:
            atomic_torch_save(out/f"epoch_{epoch}.pt",payload)
