...
run_name=stanford_area1_scale_r36_dense_log_l1_scale_supervision_effective_batch16_6epoch_20260905
...
for scene in hxp 759 1px; do
  zero_shot_dir="$project_root/results/matterport3d/${scene}_scale_r36_dense_log_l1_scale_supervision_effective_batch16_6epoch_20260905_zero_shot"
...

{
  ...
  echo "scale_teacher_mode=dense_log_l1"
...
{
  printf 'nohup setsid bash %q >/dev/null 2>&1 &\n' "$project_root/scripts/model/run_scale_r36_dense_log_l1_scale_supervision_effective_batch16_pipeline_20260905.sh"
  printf 'tail -f %q\n' "$log_path"
  printf 'cat %q\n' "$status_path"
} >"$commands_path"
...
echo "architecture=original_scale_r36_no_post_scale_adapter"
echo "scale_supervision=dense_log_l1 scale_teacher_weight=0.5 effective_batch=16"
