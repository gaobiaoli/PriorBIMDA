...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cache the deterministic scale+local BIM anchor in prepared samples"
    )
...
def main() -> None:
    ...
    for index, record in enumerate(records, 1):
        ...
        with np.load(path) as item:
            if not args.overwrite and set(ANCHOR_KEYS).issubset(item.files):
                continue
            payload = {key: item[key] for key in item.files if key not in ANCHOR_KEYS}
        ...
        payload.update(
            ...
            anchor_field=field.astype(np.float16),
            anchor_support=support.astype(np.float16),
        )
