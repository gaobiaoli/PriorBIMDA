...
def main() -> None:
    ...
    output = args.output or (
        resolve_project_path(cfg, cfg.experiment.output_dir)
        / f"reconstruction_{args.split}"
    )
    output.mkdir(parents=True, exist_ok=True)

    calibration = np.loadtxt(slabim / "calibration_files/cam_to_lidar.txt")
    pose_cache: dict[str, np.ndarray] = {}
    method_parts: dict[str, dict[str, list[np.ndarray]]] = defaultdict(
    ...
    with torch.no_grad():
        for index, record in enumerate(records):
            ...
            reference_parts[region].append(
                depth_to_world_points(gt, valid_mask=gt_valid, **common)
            )
    ...
    for region in sorted(reference_parts):
        reference = voxel_downsample(
            np.concatenate(reference_parts[region]), args.voxel_size
        )
        region_metrics = {}
        ...
        if args.save_clouds:
            save_point_cloud(
                str(region_dir / "gt_fused.ply"), reference, METHOD_COLORS["gt"]
            )
        for method in args.methods:
    ...
    for method in args.methods:
        reconstruction = voxel_downsample(
            np.concatenate(
                [
                    part
                    for region in sorted(method_parts)
                    for part in method_parts[region][method]
                ]
            ),
            args.voxel_size,
        )
